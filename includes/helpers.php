 ...
 */
	...
	// Set session data
	function gmt_edd_slack_set_session( $name, $value, $sanitize = null ) {
	...
	// Get session data
	function gmt_edd_slack_get_session( $name, $unset = false ) {
	...
	function gmt_edd_slack_get_url() {
	...
	function gmt_edd_slack_get_site_domain() {
	...
	function gmt_edd_slack_prepare_url( $url ) {
	...
	function gmt_edd_slack_clean_url( $variable, $url ) {
	...
	function gmt_edd_slack_get_redirect_url( $id, $refer = 'off' ) {
		$url = $id === 0 || $id === '0' ? get_home_url() : get_permalink( $id );
		if ( $refer === 'on' ) {
			$url = gmt_edd_slack_prepare_url( $url ) . 'referrer=' . gmt_edd_slack_get_url();
		}
		return esc_url_raw( $url );
	}

	// Set a password reset key
	function gmt_edd_slack_set_reset_key( $user_id, $url, $expires ) {

		// Add a secret, temporary key to the database
		$key = wp_generate_password( 48, false );
		set_transient( 'gmt_edd_slack_forgot_password_key_' . $key . $user_id, $key, 60 * 60 * $expires );

		// Return a URL with the reset key
		return gmt_edd_slack_prepare_url( $url ) . 'reset_pw=' . $key . $user_id;
	}
	...
	function gmt_edd_slack_has_letters( $string ) {
		return preg_match( '/[a-zA-Z]/', $string );
	}

	// Does string contain numbers?
	function gmt_edd_slack_has_numbers( $string ) {
		return preg_match( '/\d/', $string );
	}

	// Does string contain special characters?
	function gmt_edd_slack_has_special_chars( $string ) {
		return preg_match('/[^a-zA-Z\d]/', $string);
	}

	// Does string contain mixed case?
	function gmt_edd_slack_has_mixed_case( $string ) {
